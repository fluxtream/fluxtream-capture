 ...
 * 
 * When starting this service, the intent can carry the following parameters in
 * its extras: - "upload_landscape": whether landscape photos are automatically
 * uploaded - "upload_portrait": whether portrait photos are automatically
 * uploaded - "landscape_minimum_timestamp": minimum timestamp of the landscape
 * photos for automatic upload - "portrait_minimum_timestamp": minimum timestamp
 * of the landscape photos for automatic upload - "upload_url": URL at which the
 * photos will be uploaded - "authentication": a base64 encoding of
...
public class UploadService extends Service {
	...
	// The shared preferences containing the configuration and the upload status
	// of each photo
	...
	public void onCreate() {
		...
		// try {
		// Thread.sleep(1000);
		// Log.i("flx_test", "I am still alive at " +
	 ...
	 * 
	 * @param intent
	 *            The intent containing the parameters in its extras
	 */
	...
	/**
	 * Reads the photo storage library and checks if unuploaded photos should be
	 * uploaded. Returns the delay before the next check should be done.
	 */
		...
		if (uploadLandscape || uploadPortrait) {
			...
			try {
				while (cursor != null && cursor.moveToNext()) {
					...
					if (width > height) {
						// Landscape
						if (!this.uploadLandscape)
							mustBeUploaded = false;
						if (dateTaken < this.landscapeMinimumTimestamp)
							mustBeUploaded = false;
					} else {
						// Portrait
	...
	/**
	 * This thread will periodically check if new photos are available. New
	 * photos should be detected by the observer. This thread is just a useful
	 * if the observer was inactive when a new photo was taken, or if a photo is
	 * taken while another photo is being uploaded.
	 */
